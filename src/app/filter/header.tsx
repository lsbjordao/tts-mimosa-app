  ...
  function extractStringPaths(data: any[]) {
    ...
    function recurse(obj: any, currentPath: string = "") {
      ...
      if (typeof obj === 'object') {
        // Para objetos e arrays, coletamos todos os caminhos disponíveis
        for (const [key, value] of Object.entries(obj)) {
          const newPath = currentPath ? `${currentPath}.${key}` : key;
          
          if (value !== null && value !== undefined) {
            if (typeof value === 'string') {
              // Adiciona valores string
              if (!paths[newPath]) paths[newPath] = new Set();
              paths[newPath].add(value);
            } else if (typeof value === 'number' || typeof value === 'boolean') {
              // Adiciona valores numéricos e booleanos como string
              if (!paths[newPath]) paths[newPath] = new Set();
              ...
              // Também adiciona o caminho do array em si
              if (!paths[newPath]) paths[newPath] = new Set();
              paths[newPath].add(`[array with ${value.length} items]`);
            } else if (typeof value === 'object') {
              // Para objetos aninhados, continua a recursão
              recurse(value, newPath);
              // Adiciona o caminho do objeto em si
              if (!paths[newPath]) paths[newPath] = new Set();
    ...
    // Também extrai caminhos sem o índice do array
    function recurseWithoutIndex(obj: any, currentPath: string = "") {
      if (obj === null || obj === undefined) return;
        ...
        for (const [key, value] of Object.entries(obj)) {
          ...
          if (value !== null && value !== undefined) {
            if (typeof value === 'string') {
              ...
              // Para arrays, processa o primeiro item para obter a estrutura
              if (value.length > 0) {
                recurseWithoutIndex(value[0], newPath);
              }
              if (!paths[newPath]) paths[newPath] = new Set();
              paths[newPath].add(`[array]`);
            } else if (typeof value === 'object') {
              recurseWithoutIndex(value, newPath);
              if (!paths[newPath]) paths[newPath] = new Set();
              paths[newPath].add('[object]');
            }
          }
    ...
    // Processa sem índices para obter a estrutura geral
    if (data.length > 0) {
      recurseWithoutIndex(data[0]);
    }
